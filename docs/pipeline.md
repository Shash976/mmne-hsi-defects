...
`ocsvm`), each with `fit(normal)` / `score(X)`. `fit_detectors`, `score_all`,
`anomaly_map`, `flag_threshold`. The `AnomalyConfig.fit_on` switch decides what
"normal" means (`"self"` = the target's own majority, default; `"baseline"` =
bare silicon). This choice is critical — see [analysis.md](analysis.md).
...
**What:** measure each surviving region — area, perimeter, compactness, mean
spectrum, spectral variance, distance from the silicon baseline (Mahalanobis),
anomaly score — into a **region table**. Also the quantitative maps: PC score maps,
cluster map, spectral-distance map, anomaly-probability map.
...
plus `regions.spectral_distance_map`. Maps drawn by `viz.save_analysis_maps`.
